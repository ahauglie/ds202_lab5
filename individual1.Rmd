...
acc.glc <- inner_join(GLC, acc, by = "County Code")

nacc <- acc %>% group_by(STATE) %>% summarize(n=n())

n.acc.glc <- acc.glc %>% group_by(`County Code`) %>% summarize(n=n())
head(n.acc.glc)
ggplot(county, aes(x=long, y=lat))+geom_polygon(aes(group=group))+geom_point(aes(x=LONGITUD, y=LATITUDE), data=n.acc.glc %>% filter(LONGITUD <0 & LONGITUD> -130), size=0.02, color="white")
```
