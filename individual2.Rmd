...
#acc.driver.clean$`County Code` <- acc.driver.clean$COUNTY.x
#CountyData$`County Code` <- as.numeric(CountyData$`County Code`)
#str(CountyData)
#acc.driver.clean <- acc.driver.clean %>% select(`County Code`, LONGITUD, LATITUDE, DAY_WEEK, HOUR.x, SEX)
#CountyData <- CountyData %>% select(`County Code`, `County Name`)
#firstJoin <- inner_join(acc.driver.clean, CountyData, by = "County Code")
...
ggplot(county, aes(x=long, y=lat))+geom_polygon(aes(group=group))+geom_point(aes(x=LONGITUD, y=LATITUDE), data=three_join  %>% filter(LONGITUD <0 & LONGITUD > -130), size=0.0001, color="darkolive3")+facet_wrap(~MONTH)
